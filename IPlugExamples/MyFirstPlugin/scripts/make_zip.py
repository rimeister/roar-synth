...
def  main():
  ...
  installer = "\installer\MyFirstPlugin Installer.exe"
   
  if demo:
    installer = "\installer\MyFirstPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MyFirstPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "MyFirstPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MyFirstPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
