...
if %1 == 1 (echo Making MyFirstPlugin Windows DEMO VERSION distribution ...) else (echo Making MyFirstPlugin Windows FULL VERSION distribution ...)
...
msbuild MyFirstPlugin.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild MyFirstPlugin.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MyFirstPlugin.aaxplugin\Contents\Win32\MyFirstPlugin.aaxplugin --out .\build-win\aax\bin\MyFirstPlugin.aaxplugin\Contents\Win32\MyFirstPlugin.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MyFirstPlugin.aaxplugin\Contents\x64\MyFirstPlugin.aaxplugin --out .\build-win\aax\bin\MyFirstPlugin.aaxplugin\Contents\x64\MyFirstPlugin.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\MyFirstPlugin.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\MyFirstPlugin.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "MyFirstPlugin Installer" ".\installer\MyFirstPlugin Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\MyFirstPlugin Installer.exe" ".\installer\MyFirstPlugin Demo Installer.exe"
REM -del ".\installer\MyFirstPlugin Installer.exe"
REM -)
