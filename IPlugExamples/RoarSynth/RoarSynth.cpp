...
RoarSynth::RoarSynth(IPlugInstanceInfo instanceInfo)
...
void RoarSynth::NoteOnOff(IMidiMsg* pMsg)
{
  int v;
...
void RoarSynth::ProcessDoubleReplacing(double** inputs, double** outputs, int nFrames)
{
...
void RoarSynth::OnParamChange(int paramIdx)
{
  IMutexLock lock(this);
...
void RoarSynth::ProcessMidiMsg(IMidiMsg* pMsg)
{
  int status = pMsg->StatusMsg();
  int velocity = pMsg->Velocity();
...
bool RoarSynth::GetKeyStatus(int key)
{
  IMutexLock lock(this);
...
bool RoarSynth::HostRequestingAboutBox()
{
  IMutexLock lock(this);
  if(GetGUI())
  {
