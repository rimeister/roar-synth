...
def  main():
  ...
  installer = "\installer\RoarSynth Installer.exe"
   
  if demo:
    installer = "\installer\RoarSynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RoarSynth manual.pdf" 
  ]
  ...
  ZIPNAME = "RoarSynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RoarSynth-v" + FULLVERSIONSTR + "-win-demo.zip"
